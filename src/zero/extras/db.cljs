...
(z/reg-stream
  ...
  (fn [rx path & {:keys [transform]}]
    (let [path (vec path)
          watch-node-path (vec (interpose :sub path))
      ...
      (fn db-stream-cleanup []
        (let [watch-node (update (get-in @!db watch-node-path) :rxs disj rx)]
          ...
          (loop [cur-watch-node watch-node
            ...
            (cond
              ...
              (let [parent-watch-path (-> cur-watch-path pop pop)
                    parent-watch-node (-> (get-in @!db parent-watch-path)
                                        (update :sub dissoc (-> cur-watch-path pop peek)))]
                (swap! !db-watches parent-watch-path parent-watch-node)
    ...
    (letfn [(collect-all-sub-paths [cur-path watch-node]
              (cons
                ...
                (mapcat
                  ...
                  watch-node)))]
      (collect-all-sub-paths changed-path (if (empty? changed-path) @!db-watches (get-in @!db-watches (interpose :sub changed-path)))))))
...
(defn patch! [patch]
  (let [[new-db affected-paths]
    ...
    (doseq [path affected-paths
            rx (:rxs (get-in @!db-watches (interpose :sub path)))]
      (rx (get-in @!db path)))))
